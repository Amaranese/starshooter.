...
// You can write your code in this editor
	...
	/// ONCE ONLY 
	if(animationFlag =0){
	MAI = instance_create_layer(camera_get_view_x(view_camera[0]),room_height-300, "frontInstances", oZack);
	animationFlag+=1
	}
	
	// SWITCHES MAI TO TALKING MODE 
	// USEFUL maiInstance = instance_find(MAI, instance_number(MAI) - 1)
	if (instance_exists(MAI)){ if(MAI.image_index == -1 and MAI.sprite_index = sZackStart){MAI.sprite_index = sZackTalking; characterTalking = 1}}
	...
	if(characterTalking==1){
		...
		if(oGame.animationTextComplete ==1 and lineCounter == 8){myText = instance_create_layer(0,0,"Instances",oClearText); MAI.sprite_index = sZackSmiling;  oGame.animationTextComplete = 0; lineCounter +=1}
